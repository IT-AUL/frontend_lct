...
**Обновлено:** 2026-09-25 23:42 · **Фаза:** 4 — деплой (фронт + бэк) работает на VPS, типы обновлены под свежий OpenAPI бэка, полировка, заморозка
...
| CP-11 | Совместимость с живым бэком + подключение новых ручек по `docs/16` | 29.09 | 🟡 | деплой работает (run 36186113808); `schema.d.ts` обновлён под свежий OpenAPI (typecheck+301 тест ✅), 31 файл не закоммичен; ждём финальные ручки D2–D11 |
...
1. **Закоммитить** 31 изменённый файл (адаптация под свежий OpenAPI): typecheck и 301 тест зелёные, прогнать `lint + build + e2e` перед коммитом.
2. Повторить `npm run e2e:live` на живом бэке после коммита — убедиться, что новые типы совместимы с задеплоенным API.
3. Браузеры NFR-08: реальный Safari и Яндекс Браузер вручную по happy path.
