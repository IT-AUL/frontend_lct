...
**Обновлено:** 2026-09-25 22:50 · **Фаза:** 4 — фронт готов ко всем новым ручкам бэка (D2–D11), полировка, заморозка
...
- **2026-09-25 22:50** — Шлифовка UI/UX «как готовый продукт»: убран служебный текст и оправдания («сервис пока не…», «скоро»,
  «в разработке», «v1», «лимит ТЗ», версии анализатора, mock-провайдер); не показываем то, чего нет (строки «нет данных» в ДНК,
  заглушки паспорта, HTML-карточка и «скоро» в «О системе» до поддержки бэком, неактивные кнопки правки плана); тема — иконкой,
  «Провайдер моделей» → «Модели»; демо-баннер только в mock-режиме; тексты стратегий и подсказки сокращены.
  Проверки: lint, fsd, typecheck, 301 unit, build, e2e 5/5, e2e:live 3/3.
