...
**Обновлено:** 2026-09-26 23:05 · **Фаза:** 4 — аудит 5 ролями, этап 1 правок фронта в `main` и на проде, ждём доработок бэка (`docs/BACKEND_HANDOFF.md`), заморозка

**На проде сейчас:** фронт `2064374` ([Deploy #38](https://github.com/IT-AUL/frontend_lct/actions/runs/36267211001), 26.09 22:48 МСК — с правками аудита, этап 1; CI #34 зелёный), бэк — по `/api/v1/version` (последний известный `87c1a6d`;
плановая проверка каждые 20 мин сама подхватывает новые коммиты бэка из `Ranel435/LCT-prezi-2026`). Смотреть точную версию
всегда через `Actions → Deploy` в репо или `GET https://deckdna.195-209-218-212.sslip.io/api/v1/version`, не по журналу ниже — он не поспевает.
...
Проверено (26.09 21:15, ветка `new/festive-newton-hlnl7c`): lint, lint:fsd, typecheck, **350 unit-тестов** (59 файлов), build (главный чанк 468 КБ, pdf.js — отдельный ленивый чанк 495 КБ), **e2e 5/5** (Chromium) — зелёные. `e2e:live` на правках аудита ещё не гоняли (из облака бэк недоступен).
...
| CP-11 | Совместимость с живым бэком + подключение новых ручек по `docs/16` | 29.09 | 🟡 | деплой на VPS работает (`deploy/README.md`); типы под свежий OpenAPI закоммичены (`e613d88`); ждём финальные ручки D2–D11 от бэка (см. `docs/BACKEND_HANDOFF.md`) |
...
| Q-03 | VPS, HTTPS, CI | ✅ 25.09 (`deploy/README.md`); реальный домен вместо sslip.io — по желанию |
...
| 26.09 | **Никаких плашек «Демо»/«демо-режим» в интерфейсе** — фронт работает с живым бэком, интеграция проверена. Mock-режим остаётся только для тестов и разработки |
| 26.09 | Финал аудита — «0 блокеров» + дельта ошибок (а не «0 критичных»: часть ошибок бэк не чинит). Шапка на аудите/экспорте — «Ждёт решения: N критич.» |
| 26.09 | Аудит по умолчанию группируется по правилу; сырой текст бэка — в «Технических деталях» |

## 6. Следующие шаги

0. **Аудит 5 ролями (26.09) — `docs/18-ux-audit-2026-09-26.md`:** этап 1 (все P0/P1/P2 фронта) ✅ в `main`, выкачен (Deploy #38, смоук зелёный), доки синхронизированы. Осталось: прогнать `e2e:live` на правках, этап 3 — демо с LLM и пункты бэка из `docs/18` «Бэкенд / демо».
1. Дождаться от бэкендеров пунктов `docs/BACKEND_HANDOFF.md` (D1 ДНК — главный, D2/D3/D5-D11) — фронт уже готов их подхватить без правок (см. §8 хендофф).
2. Браузеры NFR-08: реальный Safari и Яндекс Браузер вручную по happy path (задеплоенный стенд — `deploy/README.md`).
3. Собрать материалы сдачи: 9 презентаций (3 варианта × 3 шаблона), видео ≤7 мин, презентация проекта (слайды 7–11 по шаблону).
4. Заморозка к 29.09 20:00 МСК, тег сдачи.

## 7. Журнал (новые записи сверху)

- **2026-09-26 23:05** — `main` подтянут (fast-forward до `a2c189b`), бэк — до `b953d69` (нативный uvicorn на :8000, `model_auto`/`contextual_audit` включены).
  OpenAPI не изменился. Зонд API по всем пунктам handoff — всё OK (DNA, plans, deck_plan, stage/timing, axes, style_fidelity,
  PNG, fingerprint/fix_preview/clipped_bbox, dry run, outcomes, экспорт r2 с HTML zip, паспорт, latest_run_id).
  Исправлено: после repair у слайдов нет `preview_artifact_id` (ленивый рендер) — фронт берёт `/slides/{id}/preview?revision=N`
  при `features.png_previews`; панель repair — кнопки в две строки (основная обрезалась).
  Проверки: lint, fsd, typecheck, 350 unit, build, e2e 5/5, e2e:live 3/3.

- **2026-09-26 21:20** — Этап 1 правок по аудиту 5 ролями (3 агента в worktree, влиты в `new/festive-newton-hlnl7c`):
  аудит на 1280 без обрезки; группировка по правилу (×N, групповой флажок, дубли слиты), список 8790 → 349 px; карточка с единицами
  и «Техническими деталями», «Только вручную»; финал «Повторный аудит: 0 блокеров» + дельта ошибок; шапка «Ждёт решения: N критич.»
  (и на экспорте); пунктирные рамки на устаревшем превью; клавиатура не перехватывает стрелки у кнопок. pdf.js лениво (главный чанк
  960 → 468 КБ), `destroy()` документов; опрос генерации с backoff 3→6→10 с и плашкой «Нет связи»; `/audit/rules` по capability;
  тосты ошибок не исчезают; sourcemap не в образе. План: «К вариантам →» главная, «Пересобрать» с подтверждением. Экспорт:
  «Открыто N ошибок · К аудиту», карточка HTML «Не реализовано сервисом». Варианты: «Критичных: N», «рекомендуем» с причиной,
  сравнение по назначению/заголовку. Плашку «Демо-режим» по решению пользователя **убрали** (§5). Доки (ARCHITECTURE, REQUIREMENTS,
  VARIANTS, AUDIT_UI) сверены с кодом. Проверки: lint, fsd, typecheck, 350 unit, build, e2e 5/5.

- **2026-09-26 18:40** — Аудит интерфейса пятью агентами (продакт 6,5 · дизайнер 6,5 · пользователь 4 · критик 3–6 · senior FE 7,5)
  на mock-стенде, 19 экранов × 2 темы. Сводка и план — `docs/18-ux-audit-2026-09-26.md`. Сквозное: финал «0 критичных»
  недостижим, 108 карточек без группировки, сырой текст бэка в карточках, демо-данные без LLM, HTML скрыт, устаревшее превью после repair.

- **2026-09-26 13:59** — Удалённый агент подтянул 25 коммитов локального агента (fast-forward, конфликтов нет):
  живая проверка на 7 шаблонах бэка, деплой на VPS с наблюдаемостью (Grafana/Vector/VictoriaMetrics),
  `docs/BACKEND_HANDOFF.md` для команды бэкенда, иконки lucide-react, правка плана до вёрстки (D7),
  журнал repair с `fingerprint` (D2), PNG-превью с откатом на PDF (D6), полировка текстов UI.
  Перепроверено с нуля: `npm ci`, lint, lint:fsd, typecheck, **301 unit-тест**, build, **e2e 5/5** (mock,
  Chromium) — всё зелёное. `deploy.yml` безопасен в этой сессии: джобы закрыты `vars.DEPLOY_HOST != ''`,
  без секретов ничего не делает даже на cron-триггере. Мелкая правка STATUS: снята устаревшая пометка
  «не закоммичено» у типов OpenAPI (коммит `e613d88` уже в истории).

- **2026-09-25 23:42** — Повторная выкатка (run 36186113808, фикс скачивания) завершена успешно: STATE=ok, 11 контейнеров
  здоровы, все цели метрик без `down`, логи поступают, Grafana — 4 дашборда, 12 алертов, источники данных.
  Недочёт с очисткой тестового проекта в smoke-тесте исправлен в свежем коммите.
  Локально: `schema.d.ts` обновлён под свежий OpenAPI бэка (новые ручки D7/D8/D10, типы `AuditIssueOut`,
  `FixPreview`, `PlanSet`, `PlanRequest`), убраны ручные расширения типов в entity-слое в пользу
  сгенерированных. 31 изменённый файл, typecheck и 301 unit-тест зелёные, не закоммичен.

- **2026-09-25 23:35** — Выкатка на VPS работает: `https://deckdna.195-209-218-212.sslip.io` (приложение) и
  `https://ops.deckdna.195-209-218-212.sslip.io` (Grafana, `/logs` — Dozzle за basic auth). Первый релиз прошёл из
  GitHub Actions: сборка образов фронта и бэка (бэк — приватный репозиторий, читается секретом `BACKEND_READ_TOKEN`),
  деплой по SSH с принудительной командой, дымовой тест на живом бэке (полный цикл до скачивания PPTX), автооткат.
  Панель проверена на живых данных: 8 групп целей метрик без `down`, логи `app` и `access`, 4 дашборда, 12 алертов.
  Исправлено по пути: права логов Caddy, симлинк `current` в `state/`, ложный сбой смоука на закрытом канале.
  Пароли — `/srv/deckdna/shared/.env` на сервере. Токен `BACKEND_READ_TOKEN` (classic PAT) отозвать после сдачи.

- **2026-09-25 23:06** — Иконки: `lucide-react` (ISC) через обёртку `shared/ui/icon` (`<Icon as={…} />`, единые размер и толщина,
  `aria-hidden`). Заменены все Юникод-символы (✓ ✕ × ← → ↑ ↓ ↳ ⇄ ☾ ▶ − +) в кнопках, чекбоксе, рельсе, конвейере, аудите,
  плане, ДНК, экспорте; добавлены иконки «Скачать», PDF/PPTX, «Заменить шаблон», «Повторный аудит» (крутится во время
  проверки), «Скопировать», «Сначала план». Проверки: lint, fsd, typecheck, 301 unit, build, e2e 5/5, e2e:live 3/3.
