...
| pages | `projects` | список проектов, создание проекта; в mock-сборке — карточка «Демо» с записанным прогоном |
...
| | `export-deck` | состояние карточки файла: создать, скачать, «устарел», «Не реализовано сервисом» |
...
- Модуль pdf.js подгружается лениво при первом превью (`import()` внутри `loadPdf`), отдельным чанком ~495 КБ;
  главный бандл — ~470 КБ. Документ загружается один раз по URL артефакта и кэшируется в кэше запросов;
  при вытеснении из кэша (держим до 6 неиспользуемых) вызывается `destroy()`, память не течёт. Каждая страница рисуется в canvas
...
Интерфейс показывает только то, что вернул API. Если данных нет, блок скрывается или честно
помечается. Правдоподобные цифры не подставляются. Что сейчас скрыто или помечено, потому что
бэкенд этого не отдаёт:

| Возможность | Что видно в интерфейсе | Где |
|---|---|---|
| HTML-экспорт (API отвечает 501) | карточка HTML с меткой «Не реализовано сервисом», без кнопки. Включится сама, когда `/capabilities` заявит формат | Экспорт |
| Соответствие шаблону (`style_fidelity` в паспорте пустой) | метрика скрыта; появится в карточке варианта, как только бэкенд её заполнит | Три варианта |
| Правка плана до вёрстки (`features.plan_only` в `/capabilities`) | без флага план только для чтения: главная кнопка «К вариантам →», второстепенная «Пересобрать с теми же данными» с подтверждением. С флагом включается редактор плана и сборка по нему | План, Бриф |
...
| PDF и паспорт новой ревизии после repair | рамки на превью пунктиром и пометка «Превью устарело: картинка rX, проблемы rY»; в экспорте файлы помечены как устаревшие, над файлами предупреждение «Открыто N ошибок · К аудиту» | Аудит, Экспорт |
| Статус каждого варианта по отдельности (async) | «О системе»: «сейчас три варианта готовы вместе» | Генерация, «О системе» |
| Текущая стадия конвейера | стадия из `job.stage` подсвечивается; если её нет, стадии показаны целиком без процентов | Генерация |
| Обрыв связи с бэкендом во время генерации | опрос не останавливается: пауза растёт 3 → 6 → 10 с, на экране плашка «Нет связи с сервисом — повторяем…» | Генерация |
| Каталог правил `/audit/rules` | запрашивается, только если `/capabilities` его заявляет; иначе локальный словарь правил | Аудит, «О системе» |
...
Mock-режим нужен для тестов (unit, e2e в CI) и разработки без бэкенда. В рабочей сборке его нет.
...
- **Unit-тесты и тесты компонентов** (Vitest, jsdom, Testing Library): `src/**/*.test.ts(x)`, 59 файлов, 350 тестов.
...
- **E2E** (Playwright, каталог `e2e/`): 5 сценариев в mock-режиме — основной путь от ДНК шаблона до
  скачивания PPTX, новый проект, тема, панели «Модели» и «О системе»; в CI — Chromium, Firefox и WebKit.
  Запуск: `npm run e2e`. Отдельно `npm run e2e:live` (`e2e-live/`) проходит полный путь на живом бэкенде.
