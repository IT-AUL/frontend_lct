...
import { alignRows } from '../lib/rows'
...
function CompareTable({ columns, auditHref }: CompareTableProps) {
  const rows = alignRows(columns.map((column) => column.slides))
  ...
  return (
    <div className={styles.root}>
      ...
      <section className={styles.table} style={gridStyle} role="table" aria-label="Слайд N во всех вариантах" aria-busy={loading}>
        ...
        {rows.map((row) => (
          <div key={row.index} className={styles.row} role="row">
            <div className={styles.index} role="rowheader">
              {formatIndex(row.index + 1)}
            </div>
            {row.cells.map((slide, column) => (
              <CompareCell key={columns[column]?.variant.id ?? column} column={columns[column]} slide={slide} auditHref={auditHref} />
            ))}
          </div>
        ))}
      </section>
...
function CompareCell({ column, slide, auditHref }: CompareCellProps) {
