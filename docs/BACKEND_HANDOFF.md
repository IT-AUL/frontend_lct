...
Фронт готов принять все пункты раздела 3 без доработок с нашей стороны — точные формы в разделе 8.
...
## 8. Что включится у нас само

**Обновлено 25.09 22:34.** Фронт подготовлен ко всем пунктам раздела 3: каждая новая ручка или поле подхватывается
без правок у нас, а пока их нет, экран работает как раньше. Ниже — точные формы, которые мы читаем. Если отдадите
иначе, напишите нам — поправим разбор за минуты.

| Ваша правка | Что мы читаем | Что включится |
|---|---|---|
| B1–B6, Q1, Q4 | — | сразу, без изменений |
| D1 | поля `design-dna` из раздела D1 | блоки «ДНК шаблона» вместо «нет данных» |
| D2 | `outcomes[]` в `GET /jobs/{id}` (в `result.outcomes`, в корне или JSON-строкой в `result_ids.outcomes`), элементы `{issue_id, status: fixed\|failed\|skipped, action, summary, reason}`; `AuditIssue.fingerprint` | журнал берёт исход и причину по каждой проблеме; «исправлено», но проблема осталась — показываем честно как «не удалось»; сопоставление ревизий по `fingerprint` |
| D2 (dry run) | `features.repair_dry_run: true`; `POST /audits/{id}/repairs?dry_run=true` (в теле тоже `dry_run: true`) → `{outcomes: [...]}` или `{job_id}` с `outcomes` в job | кнопка «Что будет сделано» с планом по каждой выбранной проблеме |
| D3 | паспорт: `metrics.style_fidelity.*`, `metrics.usage.*`, `metrics.timings.per_stage[]`, `metrics.content_support.{numbers_verified, numbers_failed, unsupported_claims}`, `metrics.readability.avg_occupancy`; `auto_fixes[]` в корне паспорта или в `metrics`; `VariantSummary.metrics.style_fidelity` (0–1) | «Соответствие» в карточке варианта (если поля нет — среднее по паспорту), блоки паспорта, раздел «Исправлено до показа» |
| D4 | `VariantSummary.{started_at, finished_at, stage}` | «Идёт · Вёрстка · 0:12», «Готово · 0:32» у каждого варианта |
| D5, D9 | `exporters: [..., "html"]` или `features.html_export` | кнопка HTML и строка «О системе» |
| D6 | `SlideInfo.preview_artifact_id` → `GET /artifacts/{id}/download` (`image/png`); `VariantSummary.montage_artifact_id` | миниатюры, сравнение, лента и инспектор аудита берут PNG, при ошибке картинки — обратно PDF; «после» в режиме «До / после» показывает слайд новой ревизии (по `SlideInfo.revision`) |
| D7 | `features.plan_only: true`; `POST /projects/{id}/plans` с телом `{template_id, content_pack_id, brief, strategies[], use_llm, provider_session_id?}` → `{plan_id, plans: [{strategy, deck_plan}]}` (или просто `DeckPlan`); `POST /projects/{id}/generations` с полем `deck_plan` (полный отредактированный `DeckPlan`) | кнопка «Сначала план» на брифе, экран правки плана (порядок, удаление, заголовки, мысли, новый слайд), «Собрать по плану»; на плане готового прогона — «Пересобрать по этому плану» |
| D8 | `GET /audit/rules` → массив или `{items|rules: [...]}` c `{code, title_ru, category, deterministic, repairable, fix_title_ru}`; `AuditIssue.fix_preview` (строка или `{description_ru}`) | названия, категории и «чинится ли» — с бэка (локальный словарь остаётся запасным); «Что будет сделано» у проблемы — из `fix_preview` |
| D10 | `Project.latest_run_id` (`latest_run_state`, `latest_run_stage` — по желанию) | список проектов и рельс шагов перестают зависеть от браузера |
| D11 | `VariantSummary.axes = {text_density, layout_diversity, visualization}` (0–1), `rationale` | полосы «Текст / Разнообразие макетов / Визуализация» и фраза в карточке — с бэка |

Ответы 404, 405 и 501 на новые ручки фронт считает «ещё не реализовано» и тихо остаётся на прежнем поведении;
5xx показывает как ошибку. После слияния мы обновим типы командой `npm run api:types:live`.
