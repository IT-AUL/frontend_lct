...
Проверено на бэкенде `Ranel435/LCT-prezi-2026`, коммит `3c466d2`, 25.09.2026.
...
| B3 | `cancel` на завершённом прогоне переводит его в `canceled` | Генерация | P0 | S |
...
| D4 | Слияние `devin/async-job-lifecycle` + статус варианта | Генерация | P1 | M |
...
Все воспроизводятся на `3c466d2`.
...
### B3. `cancel` на завершённом прогоне
```bash
curl -s -X POST "$A/generations/$RUN_ID/cancel"   # у completed-прогона: сейчас state=canceled
```
**Готово:** для `completed`, `failed` и `canceled` → `409` с `error.code = "state_conflict"`.
(В ветке `devin/async-job-lifecycle` это уже исправлено.)
...
### D4. Асинхронная генерация и статус варианта
Сейчас `POST /generations` держит соединение до конца всех вариантов (8 с без модели, минуты с моделью). В макете
экран генерации показывает статус каждого варианта (в очереди, идёт, готово · 0:32, ошибка) и таймер против 5:00.
Реализовано в ветке `devin/async-job-lifecycle` (ADR-010) — **влейте её**. Дополнительно нужно в `VariantSummary`:
`started_at`, `finished_at` и `stage` (`content` | `plan` | `compose` | `audit` | `render` | `passport`) — тогда фронт покажет
время готовности каждого варианта и текущий этап конвейера. Фронт уже умеет работать с обоими режимами
(синхронным и асинхронным).
**Готово:** `POST /generations` → 202 за <1 с; `GET /generations/{id}` показывает `queued → running → completed`
по каждому варианту с `started_at/finished_at`.
...
2. **D9, D5, D6, D4** (день): переключают строки «скоро» на «есть» без изменений на фронте.
...
| D4 (async + `started_at/finished_at`) | трекер уже работает в обоих режимах; время готовности варианта — правка в 1 месте |
